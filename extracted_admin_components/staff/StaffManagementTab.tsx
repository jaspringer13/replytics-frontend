...
export const StaffManagementTab: React.FC<StaffManagementTabProps> = ({ businessId }) => {
  const [staffMembers, setStaffMembers] = useState<StaffMember[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isInviteDialogOpen, setIsInviteDialogOpen] = useState(false);
  const [newStaffEmail, setNewStaffEmail] = useState('');
  const [newStaffName, setNewStaffName] = useState('');
  const [newStaffRole, setNewStaffRole] = useState<'manager' | 'stylist' | 'assistant'>('stylist');
  ...
  const [editStaffRole, setEditStaffRole] = useState<'manager' | 'stylist' | 'assistant'>('stylist');
  ...
  const loadStaffMembers = async () => {
    try {
      setIsLoading(true);
      const response = await apiClient.getStaff(businessId, false); // Get all staff, including inactive
  ...
  const handleInviteStaff = async () => {
    ...
    try {
      ...
      await apiClient.addStaffMember(businessId, {
  ...
  const handleRemoveStaff = async (staffId: string, staffName: string) => {
    ...
    try {
      await apiClient.removeStaffMember(businessId, staffId);
  ...
  const handleEditStaff = (member: StaffMember) => {
    ...
    setEditStaffRole(member.role as 'manager' | 'stylist' | 'assistant');
  ...
  const handleUpdateStaff = async () => {
    ...
    try {
      ...
      await apiClient.updateStaffMember(businessId, editingStaff.id, {
        name: editStaffName,
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      id="staff-role"
                      value={newStaffRole}
                      onChange={(e) => setNewStaffRole(e.target.value as 'manager' | 'stylist' | 'assistant')}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  id="editStaffRole"
                  value={editStaffRole}
                  onChange={(e) => setEditStaffRole(e.target.value as 'manager' | 'stylist' | 'assistant')}
                  disabled={isUpdating}
                  className="w-full h-10 px-3 rounded-md border border-input bg-background text-sm"
                >
                  <option value="stylist">Stylist</option>
